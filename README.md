...
1. **API密钥安全**：不要将 `.env` 文件提交到公开仓库
2. **网络要求**：需要能访问DeepSeek API和HuggingFace
3. **资源消耗**：首次运行会下载嵌入模型（约400MB）
4. **浏览器兼容**：建议使用Chrome或Edge浏览器
5. **知识库更新**：修改知识库文件后需要重启后端服务
...
### Q: 如何查看API调用情况？
A: 访问 http://localhost:8000/docs 查看API文档和测试接口。

### Q: 可以用户上传文件吗？
A: 当前版本仅支持开发者在 `knowledge_base` 目录预置资料，不支持用户临时上传。
